class Api::V1::Storefront::AccountsController < Api::V1::Storefront::ApplicationController
  ...
  def password
    ...
    if @account.valid_password?(current_password) && @account.update(password_attributes)
      render json: @account, adapter: :json
    else
      render json: ErrorResponse.new(@account), status: :unprocessable_entity
    end
