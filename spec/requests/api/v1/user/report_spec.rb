...
RSpec.describe 'api/v1/user/reports', type: :request do
  ...
  path '/api/v1/user/reports/overview' do
    get('overview report') do
      ...
      context 'filter by store id' do
        let(:store_id) { create(:store, workspace: user.current_workspace).id }

        it 'filters by store IDs' do
          Order.where(status: 'completed').limit(2).update_all(store_id: store_id)

          get overview_api_v1_user_reports_path, headers: { 'Authorization' => bearer_token_for(user) }, params: {
            store_id: store_id
          }
      ...
      context 'filter by created_by_id' do
        ...
        it 'filters by created by ID' do
          get overview_api_v1_user_reports_path, headers: { 'Authorization' => bearer_token_for(user) }, params: {
  ...
  path '/api/v1/user/reports/best_seller_products' do
    get('list best seller products') do
      ...
      parameter name: :metric, in: :query, type: :string, required: false, description: "Set ordering by metric", enum: %w[sold_quantity sales_amount]
  ...
  path '/api/v1/user/reports/best_seller_categories' do
    get('list best seller categories') do
      ...
      parameter name: :metric, in: :query, type: :string, required: false, description: "Set ordering based on metrics", enum: %w[sold_quantity sales_amount]
