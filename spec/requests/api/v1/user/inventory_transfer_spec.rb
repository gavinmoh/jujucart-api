...
RSpec.describe 'api/v1/user/inventory_transfers', type: :request do
  ...
  let(:id) { create(:inventory_transfer, :with_inventory_transfer_items, workspace: user.current_workspace).id }
  ...
  path '/api/v1/user/inventory_transfers' do
    get('list inventory transfers') do
      ...
      response(200, 'successful') do
        before do
          create_list(:inventory_transfer, 3, workspace: user.current_workspace)
        end
