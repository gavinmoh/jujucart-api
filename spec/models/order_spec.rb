...
RSpec.describe Order, type: :model do
  ...
  describe 'aasm' do
    ...
    context 'guards' do
      describe '#customer_present?' do
        let!(:order) { create(:order, :with_line_items, order_type: 'delivery') }

        it 'returns true if customer is present' do
          expect(order.send(:customer_present?)).to be(true)
        end

        it 'returns false if customer is not present' do
          order.update(customer: nil)
          expect(order.send(:customer_present?)).to be(false)
        end
      end
